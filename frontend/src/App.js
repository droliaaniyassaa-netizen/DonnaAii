...
const App = () => {
  ...
  // Helper functions for Donna's Action Plan Generation
  const getStepTitle = (step) => {
    const stepTitles = [
      "Map the Players (Organizational Awareness)",
      "Introduce a Smart Tool (Process Optimization)", 
      "Build the Circle (Influence Network)",
      "Shape Perception (Strategic Visibility)",
      "Seal the Deal (Targeted Ask)"
    ];
  ...
  const getStepDescription = (step) => {
    const stepDescriptions = [
      "Identify three key figures: your manager, their manager, and one peer with strong informal influence. Observe their communication habits, priorities, and blind spots. Shape your work so it naturally aligns with what they value.",
      "Adopt and champion a widely useful AI or productivity tool—a spreadsheet assistant, automated note-taker, or document summarizer. Position yourself as the one who enabled smoother workflows.",
      "Make yourself indispensable on your manager's projects while keeping them informed on yours. Simultaneously, build goodwill with two peers by easing their load when possible. This dual alignment ensures your name circulates upward and sideways.",
      "Tie your contributions to high-visibility initiatives leadership already tracks. Offer solutions to overlooked problems or quietly integrate new tech that improves execution. When those projects succeed, your name is linked to the win.",
      "Maintain a running \"Brag Sheet\" of quantifiable achievements: metrics improved, hours saved, revenue gained. In review meetings, frame your promotion request around evidence and a forward-looking contribution you're prepared to deliver."
    ];
    return stepDescriptions[step - 1] || "Complete this step according to your specific goals.";
  };

  const getStepInsight = (step) => {
    const stepInsights = [
      "Careers accelerate when the right three people talk about you. Design your output for their radar, not the whole office.",
      "Every time the team saves effort with this tool, they unconsciously credit you. That repetition builds relevance faster than any self-promotion.",
      "The people above see you as reliable, and the people beside you become your advocates. That combination makes you harder to ignore.",
      "Instead of chasing recognition, attach yourself to work that leadership already wants to recognize. Visibility becomes automatic.",
      "The ask becomes less \"please promote me\" and more \"here's the logical next step to expand impact.\" That tone is taken seriously."
    ];
    return stepInsights[step - 1] || "Strategic insight for this step.";
  };

  // Donna's intelligent action plan generator
  const generateDonnaActionPlan = (goalText) => {
    const goal = goalText.toLowerCase();
    
    // Business/Startup goals
    if (goal.includes('business') || goal.includes('startup') || goal.includes('company') || goal.includes('scale') || goal.includes('expand')) {
      return {
        titles: [
          "Expose the Competition (Market Differentiation)",
          "Identify Industry Choke Points (Structural Protection)", 
          "Deploy AI Marketing Tools (Efficiency Leverage)",
          "Build the Circle (Alliance Economics)",
          "Execute the Growth Strategy (Market Domination)"
        ],
        descriptions: [
          "Study competitors in detail: where they stumble—delays, poor service, inflated prices. Fix those exact gaps in your own business and emphasize them in your marketing.",
          "Every industry bleeds somewhere—supply chain delays, compliance costs, platform fees. Pinpoint the choke point in your sector and build safeguards against it.",
          "Reduce marketing costs by relying on AI. Use Jasper for ad copy, Canva AI for visuals, AdCreative.ai for tested creatives. Keep costs low, output high, and campaigns sharper than bigger competitors.",
          "Nurture your circle: Pay suppliers faster so they prioritize you. Cross-promote with non-competing brands that share your audience. Reward your top 20% repeat customers with perks and early access.",
          "Launch your refined strategy with measurable targets. Track competitor response, customer acquisition cost, and market penetration weekly. Adjust tactics based on data, not assumptions."
        ],
        insights: [
          "Consumers rarely switch brands for vague promises. They switch when you solve the one frustration that's been ignored.",
          "The businesses that control their choke points look untouchable while rivals keep losing profit to the same weakness.",
          "Professional polish doesn't have to cost like a corporate budget—AI lets small players look heavyweight.",
          "A strong circle becomes your silent marketing engine. Trust from suppliers, partners, and loyal customers triggers organic marketing faster than ads do.",
          "Markets reward consistency over perfectionism. A good strategy executed relentlessly beats a perfect strategy executed sporadically."
        ]
      };
    }
    
    // Job seeking goals  
    if (goal.includes('job') || goal.includes('hire') || goal.includes('interview') || goal.includes('internship') || goal.includes('graduate') || goal.includes('entry')) {
      return {
        titles: [
          "Map the Market (Awareness)",
          "Build the Minimum Signal (Skill Stack)",
          "Create a Sharp Profile (Visibility)",
          "Build a Warm Network (Leverage)",
          "Master the Interview (Perception)",
          "Track Wins (Strategic Ask)"
        ],
        descriptions: [
          "Research 10 companies in your target field. Note what roles they're hiring, the skills they repeat in job postings, and which tools/software appear most often.",
          "Learn the top 2–3 software tools or certifications mentioned across postings. Build 2 small projects or case studies using them.",
          "Craft a LinkedIn profile that is keyword-optimized and project-heavy. Use your summary to state what you do, not just what you studied. Add a 3–5 slide portfolio deck showcasing your best work.",
          "Instead of cold-applying endlessly, identify 5–10 professionals one level above your target role. Engage with their posts, ask thoughtful questions, and request short calls for advice.",
          "Prepare a \"3-3-1 Framework\": 3 skills you bring, 3 proof-points (projects, internships, achievements), and 1 forward-looking contribution you'd make if hired.",
          "Keep a running list of applications, interviews, feedback, and projects completed. Use this record to refine your pitch and highlight how your approach is evolving."
        ],
        insights: [
          "Job descriptions are cheat sheets—companies literally tell you the keywords that will get your résumé through filters.",
          "Even a weekend project on GitHub or a 2-page case study can prove competence better than \"course completed\" on a résumé.",
          "Recruiters scroll fast. Portfolios and concrete projects act like stop-signs—showing, not telling, wins attention.",
          "People hire names they've seen before. A small but consistent online presence makes you \"the student they already know.\"",
          "Interviews reward clarity. If you can crisply connect skills → proof → future value, you sound like someone ready to contribute on day one.",
          "Hiring managers respect candidates who learn fast. Saying \"I adjusted my portfolio after early interviews\" proves adaptability without highlighting rejection counts."
        ]
      };
    }
    
    // Default to Career Growth for most professional goals
    return {
      titles: [
        "Map the Players (Organizational Awareness)",
        "Introduce a Smart Tool (Process Optimization)", 
